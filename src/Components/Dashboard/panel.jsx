...
const Panel = () => {
    ...
    const InfoBar = ({title}) => {
        return (
            <Container fluid className={`justify-content-between d-flex align-items-center ${'shadow-sm'} mx-0 mb-4 p-3 px-lg-5`}>
                <h2 className='text-charcoal mb-0 ms-lg-2 text-capitalize'>{title}</h2>
                <div className="btn-toolbar justify-content-between align-items-center">
                    <div className="d-flex input-group focus-ring">
                        <input 
                            type="text" 
                            className="d-none d-md-inline-block focus-ring-secondary form-control rounded-start-pill border-end-0"
        ...
        <Tab.Container defaultActiveKey="explore" className='m-0'>
            <Row 
                ...
                <Col sm={2} className="px-4 d-none d-sm-flex flex-column flex-shrink-0 py-3 bg-charcoal" id='dash-sidebar'>
                    ...
                    <Nav defaultActiveKey="explore" className="flex-column mb-auto" onSelect={(eventKey) => setTabActive({eventKey: true})}>
                    {/* <li className="nav-item">
                        ...
                        <Nav.Item>
                            <Nav.Link eventKey="explore" className={`text-glitter ps-2 ${tabActive.explore ? 'border-end border-4 border-glitter':''}`}>
                                <i className="bi pe-none me-3 bi-search" width="16" height="16"></i>
                                Explore
                            </Nav.Link>
                        </Nav.Item>
                        <Nav.Item>
                            <Nav.Link eventKey="dashboard" className={`text-glitter ps-2 ${tabActive.dashboard ? 'border-end border-4 border-glitter':''}`}>
                                <i className="bi pe-none me-3 bi-speedometer" width="16" height="16"></i>
                                Dashboard
                            </Nav.Link>
                        </Nav.Item>
                        <Nav.Item>
                            <Nav.Link eventKey="history" className={`text-glitter ps-2 ${tabActive.history ? 'border-end border-4 border-glitter':''}`}>
